...
public class PuzzleSolver_AStar_H2 {
    ...
    PuzzleSolver_AStar_H2(PuzzleState initialState) throws Exception {
        this.initialState = initialState;
        generateFinalStates();
        agenda = new PriorityQueue<>(new ASTARPuzzleStateComparator());
        expanded = new ArrayList<>();
    }

    public String solvePuzzle() throws Exception {
        try {
            initialState.setHOfN(initialState.getManhattanDistance(finalState1));
            agenda.add(initialState);
            PuzzleState state;
            do {
                state = agenda.remove();
                if (PuzzleState.isEqual(finalState1, state)) {
                    return ("ASTAR_H2_FINALSTATE1_PATH: " + state.getPath() + "\nASTAR_H2_FINALSTATE1_COST: "
                            + state.getGOfN());
                }
                expanded.add(state);
                ...
                temp.appendPath("UP");
                temp.setHOfN(temp.getManhattanDistance(finalState1));
        ...
        } catch (OutOfMemoryError e1) { // If this exception is caught, it means no solution exists for first goal
                                        // state, therefore the whole procedure is started again for second goal state
            System.out.println("ASTAR_H2_FINALSTATE1_PATH: Solution not found.");
            try {
                initialState.setHOfN(initialState.getManhattanDistance(finalState2));
                agenda.add(initialState);
                PuzzleState state;
                do {
                    state = agenda.remove();
                    if (PuzzleState.isEqual(finalState2, state)) {
                        return ("ASTAR_H2_FINALSTATE2_PATH: " + state.getPath() + "\nASTAR_H2_FINALSTATE2_COST: "
                                + state.getGOfN());
                    }
                    expanded.add(state);
                    ArrayList<PuzzleState> transferredStates = new ArrayList<>();
                    PuzzleState temp = PuzzleState.performAction(state, StateAction.MOVE_UP);
                    temp.appendPath("UP");
                    temp.setHOfN(temp.getManhattanDistance(finalState2));
                    transferredStates.add(temp);
                    temp = (PuzzleState.performAction(state, StateAction.MOVE_RIGHT));
                    temp.appendPath("RIGHT");
                    temp.setHOfN(temp.getManhattanDistance(finalState2));
                ...
                return "ASTAR_H2_FINALSTATE2_PATH: Solution not found.";
            }
        }
